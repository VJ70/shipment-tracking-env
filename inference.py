...
# ── Score clamping — strictly open interval ───────────────────────────────── #

def _clamp_score(score: float) -> float:
    """Ensure score is strictly between 0.0 and 1.0 (exclusive)."""
    return round(min(max(score, _SCORE_EPSILON), 1.0 - _SCORE_EPSILON), 6)
...
def get_model_action(
    ...
    user_prompt = textwrap.dedent(f"""
        Step {step} | Last reward: {last_reward:.4f}
    ...
    try:
        ...
        if text.startswith("```"):
            ...
            if text.startswith("json"):
                text = text[4:]
        return text.strip() if text.strip() else _fallback_action(step, obs)
    except Exception as exc:
        print(f"[DEBUG] LLM call failed: {exc}", flush=True)
        return _fallback_action(step, obs)


def _fallback_action(step: int, obs: dict) -> str:
    ...
    if "close_ticket" in tools:
        return json.dumps({
            ...
            "tool_args": {"resolution_notes": "Fallback closure — shipment issue investigated."}
        })
...
async def run_episode(
    ...
    rewards: List[float] = []
    steps_taken = 0
    # Initialise score to epsilon so it is never exactly 0.0 if no steps run
    ...
    try:
        ...
        for step in range(1, max_steps + 1):
            ...
            try:
                ...
                # Clamp each step reward too — grader already does this but be safe
                reward      = _clamp_score(float(result.get("reward", _SCORE_EPSILON)))
                done        = bool(result.get("done", False))
            except Exception as exc:
                error   = str(exc)
                reward  = _SCORE_EPSILON   # never exactly 0.0
                done    = False
            ...
            history.append(f"Step {step}: {json.dumps(action_dict)[:100]} -> reward {reward:.4f}")

    finally:
        # Use last reward as episode score (grader is cumulative)
        # _clamp_score guarantees strictly (0.0, 1.0)
        final_reward = rewards[-1] if rewards else _SCORE_EPSILON
        score        = _clamp_score(final_reward)
